...
const FRICTION = 0.7; //friction coefficient of space (0 = no friction, 1 = much friction)
const GAME_LIVES = 3; //starting number of lives
const LASER_DISTANCE = 0.6; //maximum distance laser can travel as fraction of screen width
const LASER_EXPLODE_DUR = 0.1; //duration of the laser's explosion
const LASER_MAX = 10; //maximum number of lasers on screen at once
const LASER_SPEED = 500; //speed of lasers in pixels per seconds
const ROIDS_JAG = 0.4; //jaggeredness of the asteroid (0 = none, 1 = much)
const ROIDS_POINTS_LARGE = 20; //points scored for a large asteroid
const ROIDS_POINTS_MEDIUM = 50; //points scored for a medium-sized asteroid
...
fxCrash = new Sound("sounds/crash.m4a");
fxHit = new Sound("sounds/hit.m4a", 5);
fxLaser = new Sound("sounds/laser.m4a", 5, 0.1);
...
function destroyAsteroid(index) {
    ...
    //split the asteroid in two where necessary
    ...
    //start new level when no asteroid left
    if (roids.length == 0) {
        level++;
        newLevel();
    }
...
function newAsteroid(x, y, r) {
    ...
    var roid = {
        x: x,
        y: y,
        xv: Math.random() * ROIDS_SPEED * levelMultiplier / FPS * (Math.random() < 0.5 ? 1 : -1),
        yv: Math.random() * ROIDS_SPEED * levelMultiplier / FPS * (Math.random() < 0.5 ? 1 : -1),
        r: r,
...
function update() {
    ...
    for (var i = 0; i < roids.length; i++) {
        ...
        //show bounding around asteroids
        if (SHOW_BOUNDING) {
            ctx.strokeStyle = "line";
            ctx.beginPath();
    ...
    if (textAlpha >= 0) {
        ...
        ctx.font = "small-caps " + TEXT_SIZE + "px montserrat";
    ...
    ctx.textBaseline = "middle";
    ctx.fillStyle = "white";
    ctx.font = TEXT_SIZE + "px montserrat";
    ...
    ctx.textBaseline = "middle";
    ctx.fillStyle = "white";
    ctx.font = (TEXT_SIZE * 0.75) + "px montserrat";
    ctx.fillText("BEST: " + highScore, canv.width / 2 - SHIP_SIZE / 2, SHIP_SIZE);

    //detect when laser hits asteroid
    var ax, ay, ar, lx, ly;
